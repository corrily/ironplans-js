...
export function PaginatedInvoiceListFromJSONTyped(
  json: any,
  ...
  return {
    ...
    results: !exists(json, 'results')
      ? undefined
      : (json['results'] as Array<any>).map(InvoiceFromJSON),
  }
}

export function PaginatedInvoiceListToJSON(
  value?: PaginatedInvoiceList | null
): any {
  if (value === undefined) {
    return undefined
  }
  ...
  return {
    ...
    results:
      value.results === undefined
        ? undefined
        : (value.results as Array<any>).map(InvoiceToJSON),
  }
}
