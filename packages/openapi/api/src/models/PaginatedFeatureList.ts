...
export function PaginatedFeatureListFromJSONTyped(
  json: any,
  ...
  return {
    ...
    results: !exists(json, 'results')
      ? undefined
      : (json['results'] as Array<any>).map(FeatureFromJSON),
  }
}

export function PaginatedFeatureListToJSON(
  value?: PaginatedFeatureList | null
): any {
  if (value === undefined) {
    return undefined
  }
  ...
  return {
    ...
    results:
      value.results === undefined
        ? undefined
        : (value.results as Array<any>).map(FeatureToJSON),
  }
}
